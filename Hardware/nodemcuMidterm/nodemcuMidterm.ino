...
String const url = "http://ecourse.cpe.ku.ac.th:1515/";
...
void loop() {
  ...
  */
  ...
  if (cur_time - last_sent_time > 1000) {
    ...
    GET(get_builder("ipalm-exceed-plus").c_str(), get_request,server_data.plus); 
    Serial.print("plus : ");
      ...
      if (cur_buffer_length == expected_data_size) {
        switch (cur_data_header) {
          case UPDATE_PROJECT_DATA: {
              ...
              Serial.println(plus);
              POST(set_builder("ipalm-exceed-temp", temp).c_str(), update_data_to_server_callback);
              POST(set_builder("ipalm-exceed-light_lux", light_lux).c_str(), update_data_to_server_callback);
              POST(set_builder("ipalm-exceed-is_button_pressed", is_button_pressed).c_str(), update_data_to_server_callback);
              POST(set_builder("ipalm-exceed-sound", sound).c_str(), update_data_to_server_callback);
              POST(set_builder("ipalm-exceed-door", door).c_str(), update_data_to_server_callback);
              POST(set_builder("ipalm-exceed-plus", plus).c_str(), update_data_to_server_callback);
            }
            break;
          case GET_SERVER_DATA:
